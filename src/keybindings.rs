    ...
    fn default() -> Self {
        let context_bindings = hash_map_of!(
            ...
            InputContext::Global => hash_map_of!(
                ...
                KeyCode::NumpadMultiply => b!(Event::State(model::Command::MutateGlobalOctave { increment: 1 })),
                KeyCode::NumpadDivide => b!(Event::State(model::Command::MutateGlobalOctave { increment: -1 })),
                KeyCode::Escape => b!(Event::Action(event::Action::Cancel)),
                KeyCode::Enter => b!(Event::Action(event::Action::Confirm)),
